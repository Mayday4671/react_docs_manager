...
import { Button, Input, Select, Space, theme, App, Result } from 'antd';
import { SaveOutlined, CloseOutlined, FileWordOutlined } from '@ant-design/icons';
...
interface Note {
  ...
  title: string;
  content?: string;
  /** 文件类型：'md' | 'docx'，docx 类型不支持在编辑器中编辑 */
  fileType?: string;
  categoryId: number;
  tags?: string;
  pinned: number;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', padding: '16px 24px' }}>
      ...
      {note?.fileType === 'docx' ? (
        <Result
          ...
          title="Word 文档不支持在线编辑"
          subTitle="导入的 Word 文档以只读方式展示，如需修改请在本地编辑后重新导入。"
          extra={<Button onClick={onCancel}>返回查看</Button>}
        />
      ) : (
        <>
          {/* 顶部工具栏：标题、分类、标签、操作按钮 */}
